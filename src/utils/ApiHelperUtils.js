...
export const loadActivities = async (courseId) => {
  let { data } = await getApiStuff({ action: GET_ACTIVITIES, courseId: courseId });
  return (data) ? data.activities : [];
}

export const loadAnnouncements = async (courseId) => {
  let { data } = await getApiStuff({ action: GET_ANNOUNCEMENTS, courseId: courseId });
  return (data) ? data.announcements.reverse() : []; // most recent first
}

export const loadAssignments = async (courseId) => {
  let { data } = await getApiStuff({ action: GET_ASSIGNMENTS, courseId: courseId });
  return (data) ? data.assignments : [];
}

export const loadCalendar = async (courseId) => {
  const { data } = await getApiStuff({ action: GET_CALENDAR, courseId: courseId });
  let calendar = [];
  if (data) {
...
export const loadInstructors = async () => {
  let { data } = await getApiStuff({ action: GET_INSTRUCTOR_LIST });
  return (data) ? data.instructors : [];
}

export const loadLectureNotes = async (courseId) => {
  let { data } = await getApiStuff({ action: GET_LECTURE_NOTES, courseId: courseId });
  return (data) ? data.lectureNotes : [];
}

export const loadModules = async (courseId) => {
  const { data } = await getApiStuff({ action: GET_MODULES, courseId: courseId });
  return (data) ? data.modules : [];
}

export const loadOfficeHours = async (courseId) => {
  let { data } = await getApiStuff({ action: GET_OFFICE_HOURS, courseId: courseId });
  return (data) ? data.officehours : [];
}

export const loadSections = async (courseId) => {
  const { data } = await getApiStuff({ action: GET_COURSE_AND_SECTIONS, courseId: courseId });
  return (data) ? data.sections : [];
}

export const loadSectionGroups = async (courseId) => {
  const { data } = await getApiStuff({ action: GET_SECTION_GROUPS, courseId: courseId });
